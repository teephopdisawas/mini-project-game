...
init python:
    ...
    def load_faction_map():
        ...
        try:
            ...
            renpy.log('Failed to load faction_map.json: %s' % e)
            return None
    faction_map_data = load_faction_map()

screen lexicon_sidebar():
    tag lexicon
    frame style "frame":
        vbox:
            text "Lexicon" style "label_text"
            if not faction_map_data:
                text "No data loaded."
            else:
                # Show known glyphs as placeholders
                text "Unlocked Glyphs:" style "prompt_text"
                text "- Flicker (m_intro)"
                text "- Childhood Song (m_childhood)"
            textbutton "Close" action Hide('lexicon_sidebar')
...
screen world_map():
    ...
    frame:
        align (0.5, 0.5)
        if faction_map_data:
            add faction_map_data['map_image'] xalign 0.5 yalign 0.5

            # overlay hotspots
            for region in faction_map_data.get('regions', []):
                $ rx = int(region.get('x',0))
                $ ry = int(region.get('y',0))
                $ rw = int(region.get('w',40))
                $ rh = int(region.get('h',40))
                ...
                imagebutton:
                    ...
                    xsize rw
                    ysize rh
                    action [Function(renpy.store._show_region_info, region['id']), Hide('world_map')]
        else:
            text "Map not available." style "label_text"

init python:
    # helper to show region info in a quick screen
    def _show_region_info(region_id):
        data = faction_map_data or {}
        for r in data.get('regions', []):
            if r.get('id') == region_id:
                renpy.call_in_new_context('region_info', r)
                return
        renpy.notify('Region not found')

label region_info(region):
    $ desc = region.get('description', 'No description')
    ...
    menu:
        "Acknowledge":
            jump hub
        "Inspect further":
            $ consequence_map.setdefault(region.get('id'), []).append('inspected')
            e "You probe the area's glyph-signatures."
            jump hub
